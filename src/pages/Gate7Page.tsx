...
import { GATE7_TESTS, runGate7 } from '../engine/gate7';
import type { Gate7Test } from '../engine/gate7';

export function Gate7Page() {
  ...
  const total = GATE7_TESTS.length;
  const pct = Math.round((passed / total) * 100);

  const groups = [
    { name: 'General Ledger', range: [1, 5] },
    { name: 'Accounts Payable', range: [6, 16] },
    { name: 'Payments & Banking', range: [17, 23] },
    { name: 'Assets', range: [24, 27] },
    { name: 'Controlling', range: [28, 30] },
    { name: 'Period Close', range: [31, 34] },
    { name: 'Taxation', range: [35, 41] },
    { name: 'Statutory, Instruments, Legal', range: [42, 45] },
  ];
  ...
  return (
    <div className="fade-up">
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="chip !bg-acc !text-white !border-acc">PART 7 OF 10</span>
            <span className="chip">45 EXECUTABLE TESTS</span>
          </div>
          <h1 className="font-disp font-black text-[32px] leading-[1.05] tracking-tight">
            Finance · Controlling · Taxation · Statutory · Legal & Instruments
          </h1>
          <p className="text-mute mt-2 max-w-2xl">
            The financial backbone — general ledger with document splitting, accounts payable with three-way match,
            parallel depreciation, period close cockpit, overhead allocation, profitability trend, effective-dated tax,
            guarantee expiry alerts, limitation period alerts, and contingent liability provisioning.
          </p>
        </div>
        <button
          onClick={runAll}
      ...
      {groups.map((group) => {
        const tests = GATE7_TESTS.filter((t) => t.id >= group.range[0] && t.id <= group.range[1]);
      ...
      <div className="card p-5 mt-6 bg-side text-white">
        ...
        <div className="text-[12px] text-white/70 leading-relaxed">
          ...
          see the evidence. The gate passes only when all 45 tests return <span className="font-mono text-ok">pass: true</span>.
        </div>
      </div>
    </div>
  );
