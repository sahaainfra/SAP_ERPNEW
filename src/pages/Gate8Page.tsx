...
import { GATE8_TESTS, runGate8 } from '../engine/gate8';
import type { Gate8Test } from '../engine/gate8';

export function Gate8Page() {
  ...
  const total = GATE8_TESTS.length;
  const pct = Math.round((passed / total) * 100);

  const groups = [
    { name: 'HCM Lifecycle & Competency', range: [1, 5] },
    { name: 'Geo-Attendance', range: [6, 17] },
    { name: 'Labour & Payroll', range: [18, 24] },
    { name: 'Plant & Maintenance', range: [25, 33] },
    { name: 'Production', range: [34, 37] },
    { name: 'Quality & Safety', range: [38, 40] },
  ];
  ...
  return (
    <div className="fade-up">
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="chip !bg-acc !text-white !border-acc">PART 8 OF 10</span>
            <span className="chip">40 EXECUTABLE TESTS</span>
          </div>
          ...
          <p className="text-mute mt-2 max-w-2xl">
            Five operational modules — HCM with geo-attendance and anti-fraud, EAM with maintenance orders,
            PRD with RMC production and moisture correction, QMS with inspection lots and NCRs,
            EHS with permits to work and safety statistics.
          </p>
        </div>
        <button
          onClick={runAll}
      ...
      {groups.map((group) => {
        const tests = GATE8_TESTS.filter((t) => t.id >= group.range[0] && t.id <= group.range[1]);
      ...
      <div className="card p-5 mt-6 bg-side text-white">
        ...
        <div className="text-[12px] text-white/70 leading-relaxed">
          ...
          see the evidence. The gate passes only when all 40 tests return <span className="font-mono text-ok">pass: true</span>.
        </div>
      </div>
    </div>
  );
